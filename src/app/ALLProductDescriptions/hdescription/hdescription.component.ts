...
@Component({
  selector: 'app-hdescription',
  templateUrl: './hdescription.component.html',
  styleUrls: ['./hdescription.component.css']
})
export class HdescriptionComponent {
  constructor(private nexserve:NexgenservicesService,private activatedRoute:ActivatedRoute,private http:HttpClient){}

  laparray:any[]=[];
  productobj;
  productid;
  headurl:string='http://localhost:4000/headreviews'
  headarrrev:any[]=[]
...
ngOnInit()
{
  ...
  this.nexserve.getheadreviews().subscribe(val=>
    {
      this.headarrrev=val;
    })
