...
@Component({
  selector: 'app-mdescription',
  templateUrl: './mdescription.component.html',
  styleUrls: ['./mdescription.component.css']
})
export class MdescriptionComponent {
  constructor(private nexserve:NexgenservicesService,private activatedRoute:ActivatedRoute, private http:HttpClient){}

  laparray:any[]=[];
  productobj;
  productid;
  moburl:string='http://localhost:4000/mobreviews'
  mobarrreviews:any[]=[]
...
ngOnInit()
{
  ...
  this.nexserve.getmobreviews().subscribe(val=>
    {
      this.mobarrreviews=val;
    })
  ...
  this.nexserve.getallmobs().subscribe((val)=>
  {
    this.laparray=val;
